...
export default function Checkout({ config, metodos, abierto, proxApertura, onClose }) {
  ...
  // ── Local cerrado: bloqueamos el checkout con una pantalla dedicada ──────
  if (abierto === false) {
    return (
      <>
        ...
        <div className="ch-drawer">
          ...
          <div className="ch-cerrado">
            <div className="ch-cerrado-icono">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <circle cx="12" cy="12" r="10"/>
                <polyline points="12 6 12 12 16 14"/>
              </svg>
            </div>
            <h2 className="ch-cerrado-titulo">Estamos cerrados por el momento</h2>
            <p className="ch-cerrado-texto">
              {proxApertura
                ? `Volvemos a abrir ${proxApertura}.`
                : 'Todavía no podemos tomar pedidos.'}
            </p>
            <p className="ch-cerrado-texto ch-cerrado-secundario">
              Tu pedido quedó guardado en el carrito — cuando abramos, vas a poder confirmarlo.
            </p>

            <a
              className="ch-cerrado-whatsapp"
              href={`https://wa.me/${config?.whatsapp_numero}?text=${encodeURIComponent('Hola, quería hacer una consulta.')}`}
              target="_blank"
            ...
            <button className="ch-cerrado-cerrar" onClick={onClose}>Volver al menú</button>
          </div>
        </div>
